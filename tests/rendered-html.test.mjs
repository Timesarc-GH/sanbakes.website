...
test("uses compact banners, a five-card opening collection and responsive product grids", async () => {
  ...
  assert.match(css, /\.corporateSection \{[^}]*padding: 42px 6vw 72px/);
  assert.match(css, /#corporate-collection > \.dedicatedCollection \{[^}]*padding-bottom: 0/);
  assert.match(css, /\.corporateIntro \{[\s\S]*?grid-template-areas: "eyebrow eyebrow" "title intro";[\s\S]*?margin-bottom: 26px/);
  assert.match(css, /\.corporateIntro \.eyebrow \{[^}]*grid-area: eyebrow;[^}]*margin: 0/);
  assert.match(css, /\.corporateIntro h2 \{[^}]*grid-area: title;[^}]*margin: 0/);
  assert.match(css, /\.corporateIntro > p:last-child \{[^}]*grid-area: intro;[^}]*margin: 0/);
  assert.match(css, /@media \(max-width: 900px\)[\s\S]*?\.corporateIntro \{ grid-template-areas: "eyebrow" "title" "intro";/);
...
test("renders the expanded customer information pages", async () => {
  ...
  assert.equal((gifting.match(/href="#gifting-collection"/g) ?? []).length, 2);
  assert.doesNotMatch(gifting, /href="\/menu\?category=gifting"/);
  ...
  assert.match(gifting, /id="gifting-collection" class="collectionAnchor"/);
  assert.match(gifting, /Choose the gift, pack and formulation here/);
  for (const productId of ["signature-discovery-box", "reserve-collection", "brownie-tin-3-piece", "whole-brownie-tin", "seasonal-hamper"]) assert.match(gifting, new RegExp(`href="/products/${productId}"`));
  assert.equal((gifting.match(/class="menuCard"/g) ?? []).length, 5);
  assert.equal((gifting.match(/class="variantPicker"/g) ?? []).length, 5);
  assert.equal((gifting.match(/class="productPurchaseRow"/g) ?? []).length, 5);
  assert.match(gifting, /Pack, quantity &amp; formulation/);
  assert.match(gifting, /Regular \(with egg\)/);
  assert.match(gifting, /Eggless/);
  assert.equal((gifting.match(/>Add to cart<\/button>/g) ?? []).length, 5);

...
export default function PreorderPage() {
  ...
  const sendWhatsApp = (event: FormEvent<HTMLFormElement>) => {
    ...
    const message = [
      ...
      "", `PLANNING SUBTOTAL: ${formatPrice(subtotal)}`,
      hasQuotationSelection ? "Quotation item included: final total pending." : "Delivery and paid customisation are additional until confirmed.",
    ...
    <main>
      ...
      <section className="orderLayout">
        <div className="orderSummary">
          ...
          {selections.length > 0 && <div className="cartTotal"><span>{en ? "Planning subtotal" : "திட்டமிட்ட இடைக்கூட்டுத்தொகை"}</span><strong>{formatPrice(subtotal)}</strong><small>{hasQuotationSelection ? (en ? "A quotation item is included; this is not the final total." : "விலை மதிப்பீட்டு பொருள் சேர்க்கப்பட்டுள்ளது; இது இறுதி மொத்தம் அல்ல.") : (en ? "Delivery and paid customisation are added after confirmation." : "உறுதிப்படுத்திய பிறகு டெலிவரி மற்றும் கூடுதல் தனிப்பயன் சேர்க்கப்படும்.")}</small></div>}
      ...
      <section className="upiSection">
        ...
        <div className="upiCard">
          ...
          {sent && hasQuotationSelection && <div className="paymentGate"><span>!</span><p>{en ? "This cart contains a quotation item. Wait for San Bakes to replace it with a final payable total before using UPI." : "இந்த கார்ட்டில் விலை மதிப்பீட்டு பொருள் உள்ளது. UPI பயன்படுத்துவதற்கு முன் San Bakes இறுதி கட்டண தொகையை வழங்கும் வரை காத்திருக்கவும்."}</p></div>}
